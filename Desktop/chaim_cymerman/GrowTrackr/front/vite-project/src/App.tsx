    ...
    <Draggable draggableId={solder.id.toString()} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          className={`task-item ${snapshot.isDragging ? 'dragging' : ''} ${isSelected ? 'selected' : ''} ${snapshot.isDragging && draggingCount > 1 ? 'dragging-multiple' : ''}`}
          style={{
            userSelect: 'none',
      ...
      {(provided, snapshot) => (
        ...
        >
          <div className="box-header">
            ...
            <h3 className="team-name-title">{ title}</h3>
